...
author: Anthropic
---
> **In here:** running Claude Code non-interactively with `-p`/`--print` · CLI flags for output format, tool auto-approval, streaming, and session continuation · SIGTERM/background-task lifecycle and structured stream-json event schemas (`api_retry`, `system/init`, plugin/MCP load errors)
