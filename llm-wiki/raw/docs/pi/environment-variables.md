...
fetched: 2026-09-12
---
> **In here:** process-marker env vars (`AI_AGENT`, `PI_CODING_AGENT`) · shell-tool session variables (`PI_SESSION_ID`, `PI_MODEL`, etc.) · Pi process configuration variables (`PI_OFFLINE`, `PI_TELEMETRY`, proxies, terminal overrides)
...
Tools created with `createBashTool()` or `createPowerShellTool()` expose the session environment by default when registered with Pi. Injection happens before `spawnHook`, so a hook receives the variables in the hook's context object:
