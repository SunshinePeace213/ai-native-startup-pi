...
> **In here:** Host-scoped credential helpers route every HTTPS push to GitHub through the CLI's OAuth token, and GitHub refuses any push whose diff touches .github/workflows/ from a token without the workflow… · 5 claims, confidence 0.74.
...
- `clm_cc0ef2ab0add` — "Pushing the same commits over SSH succeeds because an SSH key is not an OAuth token and the workflow-scope restriction never applies to it, so repointing the remote at SSH removes the refusal permanently for that checkout" · p 0.74 · active · 1 support · 0 contradict
