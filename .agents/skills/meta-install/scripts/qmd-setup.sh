#!/usr/bin/env bash
# Build this repo's qmd search index. Idempotent — safe to re-run.
#
#   bash .agents/skills/meta-install/scripts/qmd-setup.sh
#   bash .agents/skills/meta-install/scripts/qmd-setup.sh --check   # health only, writes nothing
#   bash .agents/skills/meta-install/scripts/qmd-setup.sh --force-embed
#
# Requires the qmd CLI: bun install -g @tobilu/qmd
#
# The index is project-local: config and database live in <root>/.qmd/, not in
# the global ~/.config + ~/.cache. qmd finds it by walking up from the current
# directory, so every command run anywhere inside the repo uses this index.
#
# Two collections over the llm-wiki layer -- sibling folders, so a document is
# never indexed twice and a search scopes to one layer with -c.
#
# Every phase announces itself as `[n/6] name` so an operator (or an agent
# reading the transcript) can name the step a failure happened in.
set -euo pipefail

MODE=setup
FORCE_EMBED=0
for arg in "$@"; do
  case "$arg" in
    --check) MODE=check ;;
    --force-embed) FORCE_EMBED=1 ;;
    -h | --help)
      sed -n '2,9p' "${BASH_SOURCE[0]}" | sed 's/^# \{0,1\}//'
      exit 0
      ;;
    *)
      echo "unknown argument: $arg (expected --check, --force-embed, or --help)" >&2
      exit 2
      ;;
  esac
done

# The script lives under .agents/skills/meta-install/scripts/, so ask git for the
# root rather than counting `..` segments; fall back to the known depth when git
# is unavailable or the checkout is not a repository.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(git -C "$SCRIPT_DIR" rev-parse --show-toplevel 2> /dev/null || true)"
[ -n "$REPO_ROOT" ] || REPO_ROOT="$(cd "$SCRIPT_DIR/../../../.." && pwd)"
VAULT="$REPO_ROOT/llm-wiki"
QMD_DIR="$REPO_ROOT/.qmd"
MODELS_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/qmd/models"
# `--check` runs preflight and the health check only, so its banners count to 2.
if [ "$MODE" = check ]; then
  TOTAL=2
  HEALTH_STEP=2
else
  TOTAL=6
  HEALTH_STEP=6
fi

step() { printf '\n[%d/%d] %s\n' "$1" "$TOTAL" "$2"; }
note() { printf '      %s\n' "$*"; }
fail() {
  printf '\n✗ %s\n' "$*" >&2
  exit 1
}

# ---------------------------------------------------------------------------
# [1/6] Preflight — nothing is written before this passes.
# ---------------------------------------------------------------------------
step 1 "preflight"
command -v qmd >/dev/null 2>&1 ||
  fail "qmd not found. Install it with: bun install -g @tobilu/qmd"
[ -d "$VAULT" ] || fail "no llm-wiki/ layer at $VAULT"
note "qmd:    $(qmd --version 2>/dev/null || echo unknown) ($(command -v qmd))"
note "vault:  $VAULT"
note "index:  $QMD_DIR"
note "gpu:    QMD_LLAMA_GPU=${QMD_LLAMA_GPU:-unset (qmd default: auto)}"
note "cuda:   LD_LIBRARY_PATH=${LD_LIBRARY_PATH:-unset}"

# Reads one role out of a qmd index.yml `models:` block.
read_model() {
  awk -v want="  $2:" '
    /^models:/            { inblock = 1; next }
    inblock && /^[^ ]/    { inblock = 0 }
    inblock && index($0, want) == 1 { print $2; exit }
  ' "$1"
}

# Cache filename node-llama-cpp gives an `hf:org/repo/file.gguf` URI: hf_<org>_<file>.
cache_name() {
  case "$1" in
    hf:*)
      local rest="${1#hf:}"
      printf 'hf_%s_%s\n' "${rest%%/*}" "${rest##*/}"
      ;;
    *) basename "$1" ;;
  esac
}

config_path() {
  [ -f "$QMD_DIR/index.yaml" ] && {
    echo "$QMD_DIR/index.yaml"
    return
  }
  echo "$QMD_DIR/index.yml"
}

# ---------------------------------------------------------------------------
# Health check — cheap, read-only, and the last word on whether setup worked.
# `--check` runs this alone; a full run ends with it.
# ---------------------------------------------------------------------------
health_check() {
  step "$HEALTH_STEP" "health check"
  local config status problems=0
  config="$(config_path)"
  [ -f "$config" ] ||
    fail "no index config at $config — run: bash .agents/skills/meta-install/scripts/qmd-setup.sh"

  for role in embed generate rerank; do
    local uri file
    uri="$(read_model "$config" "$role")"
    [ -n "$uri" ] || {
      note "⚠ $role model: not recorded in $config"
      problems=$((problems + 1))
      continue
    }
    file="$MODELS_DIR/$(cache_name "$uri")"
    if [ -f "$file" ]; then
      note "✓ $role model: $(basename "$file")"
    else
      note "⚠ $role model: missing from cache ($uri)"
      problems=$((problems + 1))
    fi
  done

  status="$(qmd status 2>&1 || true)"
  printf '%s\n' "$status" | sed 's/^/      /'

  for name in wiki raw; do
    local files
    files="$(printf '%s\n' "$status" |
      awk -v want="$name (" 'index($0, want) { found = 1; next } found && /Files:/ { print $2; exit }')"
    if [ -n "$files" ] && [ "$files" -gt 0 ] 2>/dev/null; then
      note "✓ collection $name: $files files indexed"
    else
      note "⚠ collection $name: no indexed files"
      problems=$((problems + 1))
    fi
  done

  # qmd's own diagnosis: sqlite-vec, model cache, GPU probe, embedding freshness.
  local doctor
  doctor="$(qmd doctor 2>&1 || true)"
  note "qmd doctor:"
  printf '%s\n' "$doctor" | sed 's/^/        /'
  if printf '%s\n' "$doctor" | grep -q '^✗'; then
    note "⚠ qmd doctor reported a failing check"
    problems=$((problems + 1))
  fi

  if [ "$problems" -gt 0 ]; then
    printf '\n⚠ health check: %d problem(s). See docs/llm-wiki/qmd-index.md.\n' "$problems" >&2
    return 1
  fi
  printf '\n✓ qmd index ready: both collections indexed, embeddings current, models cached.\n'
}

if [ "$MODE" = check ]; then
  health_check
  exit $?
fi

# ---------------------------------------------------------------------------
# [2/6] Config — .qmd/index.yml, and the models recorded in it.
# ---------------------------------------------------------------------------
step 2 "index config and models"

# Ask qmd itself where the global index lives, before .qmd/ exists to shadow it.
# Parsing beats guessing at XDG paths, and this is qmd's own answer.
global_index=""
global_config=""
if [ ! -d "$QMD_DIR" ]; then
  # Readers consume their input to EOF: closing qmd's pipe early would kill it
  # with SIGPIPE, and `pipefail` would then abort the whole script.
  global_index="$(cd "$REPO_ROOT" && qmd status 2>/dev/null | awk '/^Index:/ && !seen {print $2; seen=1}')" || true
  config_home="${QMD_CONFIG_DIR:-${XDG_CONFIG_HOME:+$XDG_CONFIG_HOME/qmd}}"
  config_home="${config_home:-$HOME/.config/qmd}"
  [ -f "$config_home/index.yml" ] && global_config="$config_home/index.yml"
fi

# Models: an explicit env var wins — QMD_*_MODEL comes from .env via direnv, and
# it overrides both the global index's models here and the project index's
# recorded models below. Otherwise inherit whatever built the global index: a
# vector's dimension is fixed by the model that produced it, so falling back to
# qmd's smaller defaults would break the index seeded from it.
global_embed=""
if [ -n "$global_config" ]; then
  for role in embed generate rerank; do
    var="QMD_$(echo "$role" | tr '[:lower:]' '[:upper:]')_MODEL"
    inherited="$(read_model "$global_config" "$role")"
    [ "$role" = embed ] && global_embed="$inherited"
    if [ -z "${!var:-}" ] && [ -n "$inherited" ]; then
      export "$var=$inherited"
      note "inherited $role model from the global index"
    fi
  done
fi

# Seed from the global index so unchanged documents keep their vectors. Paths in
# the main checkout do not move, so this is a straight reuse: `qmd embed` below
# only has to cover documents the global index never saw. A vector row is keyed
# by the model that produced it, so once the embed model differs the seed is
# dead weight — skip the copy rather than carry rows nothing will ever match.
mkdir -p "$QMD_DIR"
if [ -n "$global_index" ] && [ -f "$global_index" ] && [ ! -f "$QMD_DIR/index.sqlite" ] &&
  [ "${QMD_EMBED_MODEL:-$global_embed}" = "$global_embed" ]; then
  cp "$global_index" "$QMD_DIR/index.sqlite"
  note "seeded $QMD_DIR/index.sqlite from $global_index"
fi

cd "$REPO_ROOT"

# `qmd init` writes .qmd/index.yml using qmd's own serializer, resolving models
# from the environment prepared above. On re-runs it keeps the recorded models.
qmd init | sed 's/^/      /'

CONFIG="$(config_path)"

# qmd resolves a model as config-first, environment-second, so an existing
# index.yml silences .env entirely — and `qmd init` only writes models into a
# config it just created. Rewrite the recorded model so re-running this script
# is what applies a .env edit. Changing the embed model invalidates every stored
# vector (they are keyed by model), so `qmd embed` below re-embeds the vault.
for role in embed generate rerank; do
  var="QMD_$(echo "$role" | tr '[:lower:]' '[:upper:]')_MODEL"
  want="${!var:-}"
  [ -n "$want" ] || continue
  have="$(read_model "$CONFIG" "$role")"
  [ "$want" = "$have" ] && continue
  awk -v line="  $role:" -v want="$want" '
    /^models:/            { inblock = 1 }
    inblock && index($0, line) == 1 { print line " " want; next }
    { print }
  ' "$CONFIG" > "$CONFIG.tmp" && mv "$CONFIG.tmp" "$CONFIG"
  note "$role model: ${have:-unset} -> $want"
done

for role in embed generate rerank; do
  note "$role: $(read_model "$CONFIG" "$role")"
done

# ---------------------------------------------------------------------------
# [3/6] Models — download only what the cache is missing, with live progress.
# ---------------------------------------------------------------------------
step 3 "model downloads"
mkdir -p "$MODELS_DIR"
missing=()
for role in embed generate rerank; do
  uri="$(read_model "$CONFIG" "$role")"
  [ -n "$uri" ] || continue
  if [ -f "$MODELS_DIR/$(cache_name "$uri")" ]; then
    note "✓ $role already cached: $(cache_name "$uri")"
  else
    note "↓ $role missing: $uri"
    missing+=("$uri")
  fi
done

if [ ${#missing[@]} -eq 0 ]; then
  note "nothing to download (${MODELS_DIR})"
else
  note "downloading ${#missing[@]} model(s) into $MODELS_DIR — percent, speed, and time left below"
  # `qmd pull --progress` prints a live bar with transfer rate and remaining
  # time, and skips any file already present at the expected size. A non-default
  # `hf:` model in a project-local config is a gated field: with no terminal to
  # ask, qmd silently skips it, so trust the local config explicitly here — the
  # config was written by this script, two lines up, not by a stranger.
  QMD_TRUST_LOCAL_CONFIG=1 qmd pull --progress ||
    fail "model download failed. Re-run this script; qmd resumes partial files."
fi

# ---------------------------------------------------------------------------
# [4/6] Collections — the two layers of llm-wiki/, plus their context blurbs.
# ---------------------------------------------------------------------------
step 4 "collections"
# Re-adding is how this stays idempotent. Embeddings are keyed by content hash,
# so dropping and re-adding a collection does not re-embed unchanged text.
for name in wiki raw sources; do
  qmd collection remove "$name" > /dev/null 2>&1 || true
done
# A collection whose path no longer exists (a checkout moved, a private segment
# not present on this machine) makes every unattended qmd run print a trust
# prompt to stdout ahead of its JSON. Drop those before re-adding the layer's own.
if [ -f "$CONFIG" ]; then
  awk '/^  [a-zA-Z0-9_-]+:$/ { name=$1; sub(":", "", name) } /^    path: / { print name, $2 }' "$CONFIG" |
    while read -r name path; do
      [ -d "$path" ] || {
        note "dropping collection $name: $path is gone"
        qmd collection remove "$name" > /dev/null 2>&1 || true
      }
    done
fi

qmd collection add "$VAULT/wiki" --name wiki --mask "**/*.md" | sed 's/^/      /'
qmd collection add "$VAULT/raw" --name raw --mask "**/*.md" | sed 's/^/      /'
note "✓ collections wiki + raw registered"

qmd context add qmd://wiki "Compiled synthesis layer: LLM-maintained wiki pages over llm-wiki/raw. Pages carry type/status frontmatter, [[wikilinks]], and cite raw archives in sources:. This is the answer layer." > /dev/null
qmd context add qmd://raw "Immutable source archives: faithful markdown captures of articles, papers, docs, chats, and other sources, filed by form. Cited by wiki pages, never edited by hand. This is the evidence layer." > /dev/null
note "✓ context blurbs attached"

# ---------------------------------------------------------------------------
# [5/6] Index — re-index the markdown, then embed what changed.
# ---------------------------------------------------------------------------
step 5 "index and embeddings"
qmd update | sed 's/^/      /'

if [ "$FORCE_EMBED" = 1 ]; then
  note "--force-embed: re-embedding the whole vault"
  qmd embed -f
else
  # A vector's dimension is fixed by the model that produced it, so switching the
  # embed model leaves `qmd embed` unable to insert into the existing table — it
  # refuses in prose rather than with an exit code, and only a full re-embed gets
  # through. Reacting to that message rather than to "did this run rewrite the
  # model" keeps the recovery idempotent: a re-run after an interrupted switch
  # still repairs the index. Any other failure propagates untouched.
  embed_status=0
  embed_log="$(qmd embed 2>&1 | tee /dev/stderr)" || embed_status=$?
  if printf '%s' "$embed_log" | grep -qi "dimension mismatch"; then
    note "embed model changed — re-embedding the whole vault"
    qmd embed -f
  elif [ "$embed_status" -ne 0 ]; then
    exit "$embed_status"
  fi
fi

health_check
