...
// swatch, a swatch widget flashes under the editor for three seconds, and the
// terminal's own background/foreground follow the theme (OSC 11/10; reset at
// shutdown). Theme files live in .pi/themes/.
...
export interface ThemeFeature {
  /** Announce the theme, sync the terminal (TUI only). */
  start(ctx: ExtensionContext): void;
  /** Catch a switch made behind the extension's back (via /settings). */
  afterAgent(ctx: ExtensionContext): void;
...
export function createThemeFeature(pi: ExtensionAPI, tui: TuiHandle): ThemeFeature {
