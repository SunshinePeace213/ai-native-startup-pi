...
// page by src/infra/render/shell.ts. It reads the data island and the page
// meta, exposes window.artifact, renders a questions form when the island
// declares questions/v1, wires hand-written [data-question]/[data-option]
// markup, keeps the page current over SSE, sends the island back as a reply
// to the version it is viewing, and reports runtime diagnostics. It holds no
// secret: its POSTs ride on the HttpOnly viewer cookie, the page's own
// origin, and a header naming the page. Plain ES2020, no dependencies, no
// requests beyond the page's own origin. Linted by the repo's ESLint
// (browser globals declared in eslint.config.mjs); never a template literal,
// because the shell ships it inside a <script>.
...
(function () {
  ...
  function request(method, path, body, keepalive) {
    var headers = { "x-artifact-page": meta.slug || "" };
  ...
  var api = {
    ...
    select: function (id, label, multi) {
      ...
      setDirty();
      syncBindings();
      if (qRoot) renderQuestions(qRoot);
      return sel;
    },
    send: function (extra) {
      return send(extra);
    ...
    return request("POST", "/publish", {
      ...
      .then(function (r) {
        ...
        if (r.status === 200 && r.body && r.body.ok) {
          lastReply = r.body.response || lastReply + 1;
          dirty = false;
          sentAt = new Date();
          setStatus(
          ...
          emit("sent", { version: version, response: lastReply });
          refreshBar();
  ...
  // ---- diagnostics: what this browser saw, reported back for verify ----------
  // The shell's probe (in <head>) has been capturing since the first byte;
  // drain what it buffered and take over its push.
  function report(level, message) {
    if (!endpoint) return;
    diagnostics.push({ level: level, message: String(message).slice(0, 2000) });
    if (diagnostics.length > 50) diagnostics.shift();
    if (!diagTimer) diagTimer = setTimeout(flushDiagnostics, 1500);
  }
  function flushDiagnostics(unloading) {
    diagTimer = null;
    if (!diagnostics.length) return;
    var rows = diagnostics.splice(0, diagnostics.length);
    request("POST", "/diagnostics", { version: version, rows: rows }, !!unloading).catch(
      function () {},
    );
  }
  var early = window.__artifactDiag;
  window.__artifactDiag = {
    push: function (row) {
      if (row) report(row.level, row.message);
    },
  };
  if (Array.isArray(early))
    early.forEach(function (row) {
      report(row.level, row.message);
    });
  ...
  function refreshBar() {
    ...
    if (p) {
      ...
      if (!sending && !(sentAt && !dirty))
        setStatus(
          dirty ? "Unsent changes" : "v" + version + (lastReply ? " · reply " + lastReply : ""),
        );
    }
  }
  function mountBar() {
  ...
  function subscribe() {
    ...
    es.onmessage = function (e) {
      ...
      if (ev.type === "response") {
        if (sending) return; // our own send
        emit("update", ev);
        if (!dirty) location.reload();
        return;
      }
      if (
        (ev.type === "version" || ev.type === "hello") &&
