...
import { threadSummary } from "../../../domain/envelope";
...
export async function comments(a: ActionContext): Promise<ToolResult> {
  ...
  return text(
    ...
    { action: a.action, slug, summary: `${threads.length} threads` },
  );
}
...
export async function resolve(a: ActionContext): Promise<ToolResult> {
  ...
  return text(`Resolved ${thread.id}.`, {
    action: a.action,
    slug,
    summary: `resolved ${thread.id}`,
  });
}
