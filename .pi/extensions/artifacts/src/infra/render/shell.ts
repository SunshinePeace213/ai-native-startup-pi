...
import type { RenderInput, RenderedPage, Renderer } from "../../app/ports";
...
/**
 * Injected in <head>, before any page script: captures console.error/warn,
 * uncaught errors, failed loads, and unhandled rejections into
 * window.__artifactDiag from the first byte, so `verify` sees what a page
 * script did before the runtime (at the end of the body) was reached. The
 * runtime drains the buffer and takes over the push.
 */
export const PROBE_SCRIPT = [
  "(function () {",
  "  var buf = [];",
  "  window.__artifactDiag = buf;",
  "  function push(level, message) {",
  "    var target = window.__artifactDiag;",
  "    var row = { level: level, message: String(message).slice(0, 2000) };",
  '    if (target && typeof target.push === "function") target.push(row);',
  "  }",
  ...
  '    else push("error", (ev.message || "uncaught error") + (ev.lineno ? " (line " + ev.lineno + ")" : ""));',
  "  }, true);",
  '  window.addEventListener("unhandledrejection", function (ev) {',
  "    var r = ev.reason;",
  '    push("error", "unhandled rejection: " + ((r && r.message) || r));',
  "  });",
  "})();",
].join("\n");
const probeTag = () => `<script id="artifact-probe">${PROBE_SCRIPT}</script>`;

/** Builds the page. Throws when the island in the source is malformed or the page is too large. */
export function buildPage(input: RenderInput): RenderedPage {
...
function inject(input: RenderInput, title: string, island: Island | null, source: string): string {
...
/** The renderer port over this shell. */
