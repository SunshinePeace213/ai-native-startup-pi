...
import { galleryPath, pagePath } from "../../shared/protocol";
...
export function galleryPage(rows: Manifest[]): string {
  const items = rows.length
    ? rows
        .map(
          (m) =>
            `<li style="padding:10px 0;border-top:1px solid #e3e6ea"><a href="${pagePath(m.slug)}" style="font-weight:600;color:#3b5bdb">${escapeHtml(m.icon ? `${m.icon} ` : "")}${escapeHtml(m.title)}</a>` +
            `<div style="color:#5b6470;font-size:.9rem">${pagePath(m.slug)} · v${m.current} · ${escapeHtml(m.updatedAt.slice(0, 16).replace("T", " "))}${m.description ? ` · ${escapeHtml(m.description)}` : ""}</div></li>`,
        )
        .join("")
