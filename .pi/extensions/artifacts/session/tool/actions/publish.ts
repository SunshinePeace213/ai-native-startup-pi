// publish — a file becomes a page, or a new version of one.
...
import { QUESTIONS_SCHEMA, declaredSchema } from "../../../shared/schemas";
import type { Island, SourceKind } from "../../../shared/types";
...
export async function publish(a: ActionContext): Promise<ToolResult> {
  ...
  return text(publishLine(result, open, file.path, a.portNote()), {
    action: a.action,
...
export async function ask(a: ActionContext): Promise<ToolResult> {
  ...
  const details = { action: a.action, slug, url: result.url, version: result.version };
  const header = publishLine(result, open, path, a.portNote());
